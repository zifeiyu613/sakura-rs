...
pub fn builder_macro_impl(input: TokenStream) -> TokenStream {
    ...
    let build_fields = fields.iter().map(|field| {
        ...
        quote! {
            ...
            #ident: self.#ident.ok_or_else(||sakura_core::errors::BuildError::MissingDependency(stringify!(#ident).to_string()))?
        }
    });
    ...
    let expanded = quote! {
        ...
        impl #impl_generics #builder_name #ty_generics #where_clause {
            ...
            // `build` 方法
            pub fn build(self) -> Result<#struct_name #ty_generics, sakura_core::errors::BuildError> {
                Ok(#struct_name {
                    #(#build_fields),*
                })
            }
