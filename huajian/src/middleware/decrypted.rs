use common::string_or_number_option;
...
pub async fn decrypt(request: Request, next: Next) -> Result<Response, AppError> {
    // 获取内容类型
        ...
        return if let Ok(form) = serde_urlencoded::from_bytes::<RequestForm>(&bytes) {
            ...
            match serde_json::from_str::<Value>(&processed_data) {
                ...
                Err(e) => {
                    ...
                    Err(AppError::Parsing(e.to_string()))
                }
            }
        } else {
            warn!("无法解析表单数据");
            Err(AppError::Parsing("无法解析表单数据".into()))
        }
